...
export default function Login() {
    ...
    const login = async (email: string, password: string) => {
        ...
        if (!!branchesResponse.isError || typeof branchesResponse.data === "string") {
            ...
            alertService.error(`Error cargando los datos de las sucursales: ${message}`);
            setError(true);
            return;
        }
        ...
        if (!!categoriesResponse.isError || typeof categoriesResponse.data === "string") {
            const message = !!categoriesResponse.exception
                ? categoriesResponse.exception.message
                : categoriesResponse.error?.message;
            alertService.error(`Error cargando los datos de los productos: ${message}`);
            setError(true);
            return;
        }
        ...
        const branchList = branchesResponse.data!.branches.filter((b) => !b.branch.deleted);
